import { createServer, type IncomingMessage, type ServerResponse } from "node:http";
import { readAudit } from "../audit";
import {
  commands, saveState,
  type BotState, type EconomySettings, DEFAULT_ECONOMY,
} from "../commands/index";
import { PAGE } from "./ui";
import type { Client } from "discord.js";

/**
 * Owner web panel API: localhost dashboard to edit commands, economy settings,
 * verification config, view servers and live stats. No external dependencies.
 */

let panelState: BotState | null = null;
let panelClient: Client | null = null;

export function setPanelState(state: BotState) { panelState = state; }
export function setPanelClient(client: Client) { panelClient = client; }

export interface PanelStat {
  name: string;
  stats: "idle" | "thinking" | "working" | "talking" | "alert";
  activity: string;
  since: string;
}

function guilds() {
  if (!panelClient) return [];
  return [...panelClient.guilds.cache.values()].map((g) => ({
    id: g.id,
    name: g.name,
    memberCount: g.memberCount ?? 0,
    roles: [...g.roles.cache.values()]
      .filter((r) => r.id !== g.id)
      .map((r) => ({ id: r.id, name: r.name })),
    channels: [...g.channels.cache.values()]
      .filter((c) => (c as any).isTextBased?.())
      .map((c) => ({ id: c.id, name: c.name })),
  }));
}

function overview() {
  const st = panelState;
  const ecoAccounts = st ? Object.keys(st.eco).length : 0;
  const verified = st ? Object.values(st.verify).reduce((n, v) => n + Object.keys(v.verified).length, 0) : 0;
  const gs = guilds();
  const up = process.uptime();
  return {
    stats: {
      uptime: Math.floor(up / 3600) + "h " + Math.floor((up % 3600) / 60) + "m",
      memory: (process.memoryUsage().heapUsed / 1024 / 1024).toFixed(1) + " MB",
      node: process.version,
      guildCount: gs.length,
      totalMembers: gs.reduce((n, g) => n + g.memberCount, 0),
      commandCount: commands.length,
      ecoAccounts,
      verified,
    },
    guilds: gs,
    audit: readAudit(50),
  };
}

function json(res: ServerResponse, code: number, data: unknown) {
  res.writeHead(code, { "content-type": "application/json" });
  res.end(JSON.stringify(data));
}

async function body(req: IncomingMessage): Promise<any> {
  const chunks: Buffer[] = [];
  for await (const c of req) chunks.push(c as Buffer);
  try { return JSON.parse(Buffer.concat(chunks).toString("utf8") || "{}"); } catch { return {}; }
}

function handleApi(req: IncomingMessage, res: ServerResponse, url: URL): boolean {
  const p = url.pathname;
  if (p === "/api/overview" && req.method === "GET") return json(res, 200, overview()), true;
  if (p === "/api/commands" && req.method === "GET") {
    const st = panelState;
    const list = commands.map((c) => ({
      name: c.name, category: c.category, description: c.description, usage: c.usage,
      perm: c.perm !== undefined, modOnly: c.modOnly === true,
      enabled: st ? st.commandOverrides[c.name]?.enabled !== false : true,
    }));
    return json(res, 200, { commands: list }), true;
  }
  if (p === "/api/commands/override" && req.method === "POST") {
    if (!panelState) return json(res, 500, { error: "state not ready" }), true;
    const b = await body(req);
    if (typeof b.name !== "string" || !commands.find((c) => c.name === b.name)) {
      return json(res, 400, { error: "unknown command" }), true;
    }
    const cur = panelState.commandOverrides[b.name] ?? { enabled: true };
    panelState.commandOverrides[b.name] = {
      ...cur,
      enabled: typeof b.enabled === "boolean" ? b.enabled : cur.enabled,
      modOnly: typeof b.modOnly === "boolean" ? b.modOnly : cur.modOnly,
    };
    saveState(panelState);
    return json(res, 200, { ok: true }), true;
  }
  if (p === "/api/economy" && req.method === "GET") {
    return json(res, 200, panelState?.economy ?? DEFAULT_ECONOMY), true;
  }
  if (p === "/api/economy" && req.method === "POST") {
    if (!panelState) return json(res, 500, { error: "state not ready" }), true;
    const b = await body(req);
    const s = panelState.economy;
    for (const k of Object.keys(DEFAULT_ECONOMY) as (keyof EconomySettings)[]) {
      const v = b[k];
      if (typeof v === "number" && Number.isFinite(v) && v >= 0) (s as any)[k] = v;
    }
    saveState(panelState);
    return json(res, 200, { ok: true, economy: s }), true;
  }
  if (p === "/api/verify" && req.method === "GET") {
    const st = panelState;
    const out: Record<string, any> = {};
    for (const g of guilds()) {
      const cfg = st?.verify[g.id];
      out[g.id] = {
        enabled: cfg?.enabled ?? false,
        roleId: cfg?.roleId ?? null,
        logChannelId: cfg?.logChannelId ?? null,
        verifiedCount: cfg ? Object.keys(cfg.verified).length : 0,
        verified: cfg ? Object.entries(cfg.verified).slice(-50).map(([id, r]) => ({ id, ...r })) : [],
      };
    }
    return json(res, 200, { guilds: guilds(), verify: out }), true;
  }
  if (p === "/api/verify" && req.method === "POST") {
    if (!panelState) return json(res, 500, { error: "state not ready" }), true;
    const b = await body(req);
    if (typeof b.guildId !== "string") return json(res, 400, { error: "guildId required" }), true;
    const cfg = panelState.verify[b.guildId] ??= { enabled: false, verified: {} };
    if (typeof b.enabled === "boolean") cfg.enabled = b.enabled;
    if (typeof b.roleId === "string" || b.roleId === null) cfg.roleId = b.roleId ?? undefined;
    if (typeof b.logChannelId === "string" || b.logChannelId === null) cfg.logChannelId = b.logChannelId ?? undefined;
    saveState(panelState);
    return json(res, 200, { ok: true }), true;
  }
  return false;
}

/** Start the owner control panel on http://localhost:PORT (default 4173). */
export function startPanel(port = Number(process.env.PANEL_PORT ?? 4173)): void {
  const server = createServer(async (req, res) => {
    const url = new URL(req.url ?? "/", "http://localhost");
    try {
      if (await handleApi(req, res, url)) return;
    } catch (e) {
      return json(res, 500, { error: String(e) });
    }
    res.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    res.end(PAGE);
  });
  server.listen(port, () => {
    console.log(`Owner control panel: http://localhost:${port}  (set PANEL_PORT to change)`);
  });
}
