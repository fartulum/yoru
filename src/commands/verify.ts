...
import { base, ok, fail, requirePerm } from "./shared.js";
import type { BotCommand } from "../types.js";

/**
 * Verification system: members run !verify to receive the server's verify role.
 * Configurable per guild (role, log channel) via commands or the owner web panel.
 */
...
export const verifyCommands: BotCommand[] = [
  {
    ...
    async run(ctx) {
      ...
      await ok(ctx.msg, "admin", "Verified", "You're in. Welcome to the server!");
    },
  },
  {
    name: "setverifyrole",
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      e.addFields(
        ...
        { name: "Verified members", value: String(Object.keys(cfg.verified).length), inline: true },
      );
